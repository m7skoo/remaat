...
class SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    SharedPreferences.getInstance().then((prefs) {
      setState(() {
        ...
        driverId = prefs.getInt('0')!;
      });
      return prefs.getString("email")!;
    });
    _fetchData(token, driverId);
    print(token);
    print(driverId);
  ...
  Future<void> _fetchData(token1, driverId1) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    setState(() {
      token1 = prefs.getString(ShareConst.token);
      driverId1 = prefs.getInt(ShareConst.driverId.toString())!;
    });
    getDataAccept(token1, driverId1).then((subjectFromServer) {
      setState(() {
  ...
  List<AcceptOrder> ulist = [];
  List<AcceptOrder> userLists = [];
  //API call for All Subject List

  Future<List<AcceptOrder>> getDataAccept(pathtoken, driverid) async {
